...
def fetch_data(key_auth, github_url, repo_id, logger, tag_only = False):
    ...
    data = request_graphql_dict(key_auth, url, logger, query)
...
def releases_model(augur_db, repo_git, repo_id, key_auth, logger):
    ...
    if 'releases' in data:
        if 'edges' in data['releases'] and data['releases']['edges']:
            ...
            data = fetch_data(key_auth,repo_git, repo_id, logger, True)
