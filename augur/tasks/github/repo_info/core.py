...
def query_committers_count(key_auth, owner, repo, logger):
...
def is_forked(key_auth, owner, repo, logger): #/repos/:owner/:repo parent
...
def is_archived(key_auth, owner, repo, logger):
    logger.info('Querying committers count\n')
    url = f'https://api.github.com/repos/{owner}/{repo}'
...
def grab_repo_info_from_graphql_endpoint(key_auth,query, logger):
...
def repo_info_model(key_auth, repo_orm_obj, logger, augur_db_engine):
    ...
    try:
        data = grab_repo_info_from_graphql_endpoint(key_auth, query, logger)
    ...
    committers_count = query_committers_count(key_auth, owner, repo, logger)
