...
def git_repo_initialize(facade_db, repo_base_directory, repo_git):
    ...
    row = Repo.get_by_repo_git(facade_db.session, repo_git)
...
# Update existing repos
    ...
    while attempt < 2:
        ...
        try:
            ...
            firstpull = (f"git -C {repo_base_directory}{row['repo_group_id']}/{row['repo_path']}{row['repo_name']} pull")
...
#                    session.log_activity('Verbose', f'remote default is {logremotedefault}.')
                ...
                getremotedefault = (f"git -C {repo_base_directory}{row['repo_group_id']}/{row['repo_path']}{row['repo_name']} remote show origin | sed -n '/HEAD branch/s/.*: //p'")
...
#                session.log_activity('Verbose', f'remote default is {logremotedefault}.')
            ...
            try: 
                ...
                cmd_reset = (f"git -C {repo_base_directory}{row['repo_group_id']}/{row['repo_path']}{row['repo_name']} reset --hard origin")

                cmd_reset_wait = subprocess.Popen([cmd_reset],shell=True).wait()

                cmd_clean = (f"git -C {repo_base_directory}{row['repo_group_id']}/{row['repo_path']}{row['repo_name']} clean -df")
