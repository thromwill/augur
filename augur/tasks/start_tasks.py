...
def task_failed(request,exc,traceback):
    ...
    with GithubTaskManifest(logger) as manifest:
        ...
        query = augur_db.session.query(CollectionStatus).filter(or_(core_id_match,secondary_id_match,facade_id_match))
...
def start_facade_collection(logger, session,max_repo,days):
    ...
    logger.info(f"Starting facade collection on {len(repo_git_identifiers)} repos")

    logger.info(f"Facade collection starting for: {tuple(repo_git_identifiers)}")
