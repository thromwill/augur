...
def dei_track_repo(application: ClientApplication):
    ...
    if not repo:
        ...
        ret["log"].append("Repo added, collection started")
    else:
        ret["log"].append("Repo already exists")
        repo_id = repo.repo_id
    
    badges = session.query(BadgingDEI).filter(BadgingDEI.badging_id == dei_id).all()
    ...
    if not badges:
        ...
        ret["log"].append("Badge ID already exists")
        ret["log"].append(badges)
