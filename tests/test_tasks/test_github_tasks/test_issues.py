...
from augur.tasks.github.issues.core import *
from tests.test_tasks.test_github_tasks.issue_data import issue_data
...
def test_extract_data_from_issue():
    ...
    for github_issue_dict in issue_data:

        augur_issue_dict, labels, assignees, contributors = extract_data_from_issue(github_issue_dict, repo_id, tool_source, tool_version, data_source)

        assert augur_issue_dict is not None
        assert labels is not None
        assert assignees is not None
        assert contributors is not None

        assert type(augur_issue_dict) == dict
        assert len(labels) == len(github_issue_dict["labels"])
        assert len(assignees) == len(github_issue_dict["assignees"])
        assert len(contributors) == 1
        

# def test_extract_data_from_issue_list():
...
# def test_insert_issue_contributors(github_api_key_headers, test_db_session, pr_number):
...
# def test_insert_issues(github_api_key_headers, test_db_session, repo):
...
# def test_map_other_issue_data_to_issue(github_api_key_headers):
